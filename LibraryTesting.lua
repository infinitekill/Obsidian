...
do
    ...
    function Funcs:AddPriorityDropdown(Idx, Info)
        ...
        local Holder = New("Frame", {
            BackgroundTransparency = 1,
            Size = UDim2.new(1, 0, 0, PriorityDropdown.Text and 39 or 21),
            Visible = PriorityDropdown.Visible,
            Parent = Container,
        })
        ...
        local Display = New("TextButton", {
            Active = not PriorityDropdown.Disabled,
            AnchorPoint = Vector2.new(0, 1),
            BackgroundColor3 = "MainColor",
            BorderColor3 = "OutlineColor",
            BorderSizePixel = 1,
        ...
        PriorityDropdown.Menu = MenuTable
        Library:UpdateDPI(MenuTable.Menu, {
            Position = false,
            Size = false,
        })
    
        function PriorityDropdown:RecalculateListSize(Count)
            local Y = math.clamp(
                (Count or GetTableSize(PriorityDropdown.Values)) * (21 * Library.DPIScale),
                0,
                Info.MaxVisibleDropdownItems * (21 * Library.DPIScale)
            )
        ...
        function PriorityDropdown:UpdateColors()
            if Library.Unloaded then
                return
            end
    
            Label.TextTransparency = PriorityDropdown.Disabled and 0.8 or 0
            Display.TextTransparency = PriorityDropdown.Disabled and 0.8 or 0
            ArrowImage.ImageTransparency = PriorityDropdown.Disabled and 0.8 or MenuTable.Active and 0 or 0.5
        end
    
        function PriorityDropdown:Display()
            if Library.Unloaded then
                return
            end
            Display.Text = table.concat(PriorityDropdown.Values, ", ")
        end
    
        function PriorityDropdown:OnChanged(Func)
            PriorityDropdown.Changed = Func
        end
    
        local Buttons = {}
        function PriorityDropdown:BuildDropdownList()
            local Values = PriorityDropdown.Values
            ...
            for Index, Value in pairs(Values) do
                ...
                local UpArrowButton = New("ImageButton", {
                    AnchorPoint = Vector2.new(1, 0.5),
                    Position = UDim2.fromScale(1, 0.5),
                    Size = UDim2.fromOffset(16, 16),
                    Image = ArrowIcon and ArrowIcon.Url or "",
                ...
                local DownArrowButton = New("ImageButton", {
                    AnchorPoint = Vector2.new(1, 0.5),
                    Position = UDim2.fromOffset(Display.AbsoluteSize.X - 24, 0.5 * Button.AbsoluteSize.Y),
                    Size = UDim2.fromOffset(16, 16),
                    Image = ArrowIcon and ArrowIcon.Url or "",
                    ImageRectOffset = ArrowIcon and ArrowIcon.ImageRectOffset or Vector2.zero,
                    ImageRectSize = ArrowIcon and ArrowIcon.ImageRectSize or Vector2.zero,
                ...
                UpArrowButton.MouseButton1Click:Connect(function()
                    if Index > 1 then
                        local temp = PriorityDropdown.Values[Index]
                        PriorityDropdown.Values[Index] = PriorityDropdown.Values[Index - 1]
                        PriorityDropdown.Values[Index - 1] = temp
                    else
                        local temp = table.remove(PriorityDropdown.Values, Index)
                        table.insert(PriorityDropdown.Values, #PriorityDropdown.Values + 1, temp)
                    end
                    PriorityDropdown:BuildDropdownList()
                    PriorityDropdown:Display()
                    Library:SafeCallback(PriorityDropdown.Callback, PriorityDropdown.Values)
                    Library:SafeCallback(PriorityDropdown.Changed, PriorityDropdown.Values)
                end)
    
                DownArrowButton.MouseButton1Click:Connect(function()
                    if Index < #PriorityDropdown.Values then
                        local temp = PriorityDropdown.Values[Index]
                        PriorityDropdown.Values[Index] = PriorityDropdown.Values[Index + 1]
                        PriorityDropdown.Values[Index + 1] = temp
                    else
                        local temp = table.remove(PriorityDropdown.Values, Index)
                        table.insert(PriorityDropdown.Values, 1, temp)
                    end
                    PriorityDropdown:BuildDropdownList()
                    PriorityDropdown:Display()
                    Library:SafeCallback(PriorityDropdown.Callback, PriorityDropdown.Values)
            ...
            PriorityDropdown:RecalculateListSize(Count)
        end
    
        function PriorityDropdown:SetValues(Values)
            PriorityDropdown.Values = Values
            PriorityDropdown:BuildDropdownList()
            PriorityDropdown:Display()
            Library:SafeCallback(PriorityDropdown.Callback, PriorityDropdown.Values)
            Library:SafeCallback(PriorityDropdown.Changed, PriorityDropdown.Values)
        ...
        function PriorityDropdown:SetDisabled(Disabled: boolean)
            PriorityDropdown.Disabled = Disabled
    
            if PriorityDropdown.TooltipTable then
                PriorityDropdown.TooltipTable.Disabled = PriorityDropdown.Disabled
            end
    
            MenuTable:Close()
            Display.Active = not PriorityDropdown.Disabled
            PriorityDropdown:UpdateColors()
        end
    
        function PriorityDropdown:SetVisible(Visible: boolean)
            PriorityDropdown.Visible = Visible
    
            Holder.Visible = PriorityDropdown.Visible
            Groupbox:Resize()
        end
    
        function PriorityDropdown:SetText(Text: string)
            PriorityDropdown.Text = Text
            Holder.Size = UDim2.new(1, 0, 0, (Text and 39 or 21) * Library.DPIScale)
        ...
        Display.MouseButton1Click:Connect(function()
            if PriorityDropdown.Disabled then
                return
            end
            MenuTable:Toggle()
        end)
    
        if typeof(PriorityDropdown.Tooltip) == "string" or typeof(PriorityDropdown.DisabledTooltip) == "string" then
            PriorityDropdown.TooltipTable = Library:AddTooltip(PriorityDropdown.Tooltip, PriorityDropdown.DisabledTooltip, Display)
            PriorityDropdown.TooltipTable.Disabled = PriorityDropdown.Disabled
        end
        ...
        PriorityDropdown.Holder = Holder
        table.insert(Groupbox.Elements, PriorityDropdown)
    
        Options[Idx] = PriorityDropdown
...
do
    ...
    ThemeManager.BuiltInThemes = {
        ['Default'] 	    = { 1, httpService:JSONDecode([[{"FontColor":"ffffff","MainColor":"252526","AccentColor":"007acc","BackgroundColor":"1e1e1e","OutlineColor":"333333"}]]) },
        ...
        ["Obsidian"] 		= { 9, httpService:JSONDecode([[{"FontColor":"ffffff","MainColor":"191919","AccentColor":"7d55ff","BackgroundColor":"0f0f0f","OutlineColor":"282828"}]]) },
